...
* `Worker count`, `Auto job per worker` and `Auto min worker`: these parameters control the ideal number of workers needed to process jobs from a given queue. Given a queue with `Q` number of pending jobs, the number of workers started by the daemon will be:

    `Number of running worker in auto mode`= min( `Worker_count`, max( `Auto min worker`, ceil( `Q` / `Auto job per worker` ) ) )
